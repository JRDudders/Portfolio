@echo off
REM Aggressive fix for Pydantic version conflicts in Conda environments (Windows)
...
REM Step 1: Uninstall ALL potentially conflicting packages
echo Step 1: Removing all potentially conflicting packages...
pip uninstall -y pydantic pydantic-core pydantic-settings fastapi uvicorn starlette typing-extensions annotated-types 2>nul

echo Old packages removed
echo.

REM Step 2: Clear pip cache
echo Step 2: Clearing pip cache...
pip cache purge 2>nul

echo.

REM Step 3: Install core dependencies with exact versions
echo Step 3: Installing core dependencies with pinned versions...
pip install --no-cache-dir typing-extensions==4.12.2 annotated-types==0.7.0 pydantic-core==2.27.1 pydantic==2.10.3
...
REM Step 4: Install FastAPI and Uvicorn
echo Step 4: Installing FastAPI and Uvicorn...
pip install --no-cache-dir starlette==0.41.3 fastapi==0.115.5 uvicorn==0.32.1 python-multipart==0.0.18
echo.
